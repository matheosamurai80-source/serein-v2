 ...
 */
/**
 * Photo d'une liste de courses (manuscrite ou imprimée) → noms d'articles.
 * On enlève puces, cases à cocher, numéros et quantités en tête ; on garde
 * les vraies lignes d'articles (au moins 2 lettres, pas trop longues). Dédoublonné.
 */
export function parseShoppingListPhoto(text) {
  const out = []; const seen = new Set()
  for (const raw of String(text ?? '').split('\n')) {
    let line = norm(raw)
    line = line.replace(/^[-–—•*·▢□☐✓✔o☑]+\s*/i, '')  // puces / cases
      .replace(/^\d+\s*[.)]\s*/, '')                   // « 1) », « 2. »
      .replace(/^\d+\s*[xX*]\s*/, '')                  // « 2x », « 3 * »
      .trim()
    if (line.length < 2 || line.length > 40) continue
    if ((line.match(/[a-zA-ZÀ-ÿ]/g) ?? []).length < 2) continue // au moins 2 lettres
