...
> Dernière mise à jour : 2026-07-14 (Brique A — Accueil « À faire » + barre d'onglets basse)
>
...
| 2026-07-14 | **Brique A — Accueil « À faire » + onglets bas** : `construireAFaire(donnees, aujourdhui)` pur (`src/lib/accueil/logic.ts`) agrège résiliation (commitments) / échéance (factures_ponctuelles) / rappel (reminders) / fin de garantie (localStorage, **pas de table**) → 5 cartes max, tri criticité×proximité (retard en tête, horizon 30 j) ; page `/accueil` (atterrissage post-connexion, état vide « Rien d'urgent ») ; `FoyerTabs` en **barre basse fixe**. Via couche services, aucune table, aucun module métier touché | Sandbox `test-afaire.mjs` 14/14, suite 718 PASS 0 FAIL, lint 0, build vert (typecheck OK) |
| 2026-07-14 | Le « + » lit les **PHOTOS** de documents + amende réelle (retour Juju « pas fonctionnel à photo ») : ① `extractImageText()` (image → prétraitement N&B → OCR Tesseract local, CDN de secours) + le « + » accepte `image/*` (plus seulement PDF) ; ② sur la vraie amende de Juju : routeur reconnaît amende/contravention/antai/impôt/CAF… → **démarche** ; `detectOfficialDoc` amende → **2 liens officiels** : payer (amendes.gouv.fr) + contester (antai.gouv.fr), rendus dans le « + » | Sandbox 695 PASS 0 FAIL (officiel + router MAJ, amende→démarche), lint 0, build vert |
| 2026-07-14 | **Pavé Documents officiels** (choix Juju) : `detectOfficialDoc(text)` (pur, `src/lib/officiel/logic.ts`) reconnaît amende→ANTAI, impôt/taxes→impots.gouv, CAF, ameli, URSSAF, France Travail, carte grise→ANTS (repli service-public) ; intégré au « + » : une démarche = doc officiel affiche le **lien officiel vers la solution** (lettre en secondaire). Liens .gouv only, zéro partenaire | Sandbox `officiel` 11/11, suite 694 PASS 0 FAIL, lint 0, build vert |
| 2026-07-14 | Garanties — **conservation de la preuve** (retour Juju) : photo du ticket/facture jointe à chaque garantie, **compressée** (`scaledDimensions` pur, max 1400 px + JPEG 0,7) et stockée **en local dans IndexedDB** (`photos.ts`, rien ne quitte l'appareil) ; ajout via 📷 (appareil photo/import), affichage « voir le ticket » par garantie, suppression liée | Sandbox `equipment` 18/18 (dont redimensionnement), suite 683 PASS 0 FAIL, lint 0, build vert |
| 2026-07-14 | **Nouveau pavé Équipement / Garanties** (choix Juju) : `src/lib/equipment/logic.ts` (`warrantyEnd` +24 mois légal, `warrantyStatus` urgence, `extractPurchaseInfo` enseigne/date/prix — pur) + store localStorage + page `/garanties` (ajout manuel ou collage de ticket, liste triée par urgence, alerte avant fin de garantie) + carte dans Mon foyer. « Arme le client », zéro partenaire | Sandbox `equipment` 14/14, suite 679 PASS 0 FAIL, lint 0 erreur, build vert (route `/garanties`) |
| 2026-07-14 | Analyse relevé — liste propre par défaut (retour Juju) : `buildSuggestions` expose `recognized` (`category !== 'other'` = enseigne identifiée) ; `/analyse` n'affiche + ne coche + ne totalise que les **abonnements reconnus** (Orange/AXA/EDF/Canal…), et masque le bruit « Autre » (achats ponctuels, courses) derrière un repli « + Voir N autres opérations » | Sur le vrai relevé : 4 reconnus visibles, 9 « Autre » repliés ; `analyse.test.ts` +1 (recognized), suite 664 PASS, lint 0, build vert |
| 2026-07-14 | Parseur relevé — faux positifs & montants (retour Juju « SFR/MMA que j'ai pas, EDF à 2300 ») : ① `resolveMerchant` matche désormais en **frontière de mot** → « sfr » du BIC `BOUSFRPP` et « mma » de `LA MAMMA` (pizzeria) ne créent plus de faux abonnement ; ② `AMOUNT_RE` avec `(?<!\d)` → un montant ne démarre plus au milieu d'un nombre (mandat `…180002` + `301,16` ne se lit plus `2 301,16`) → **EDF 301,16 €** au lieu de 2301. Vrais abos propres : Orange 6×, AXA 6×, EDF, Canal+ | Sandbox `parser-releve-multiligne` 10/10 (BIC≠SFR, MAMMA≠MMA, EDF 301,16), suite 664 PASS 0 FAIL, lint 0, build vert |
| 2026-07-14 | **Lecture PDF fiabilisée** (retour Juju « n'arrive pas à lire ») : pdf.js échoue souvent sur mobile → `extractPdfTextResilient()` = navigateur d'abord (fichier local), **secours serveur** `POST /api/pdf/extract` (pdf-parse, en mémoire, AUCUN stockage) sinon. `serverExternalPackages: ['pdf-parse']`. Utilisé par `/ajouter` et `/analyse` | Route vérifiée en vrai (prod `npm start` + POST du relevé réel → `{ok, text}` 18936 car.), lint 0, build vert (route générée), suite 661 PASS |
| 2026-07-14 | **Parseur de relevé corrigé** (vrai relevé Société Générale, « relevé bloquant ») : ① 2 dates par ligne (Date+Valeur) → l'année « 2026 » ne devient plus le marchand (retrait de TOUTES les dates + réfs carte X#### + longues réfs) ; ② prélèvements **multi-lignes** (en-tête daté → `DE:`/`MOTIF`/`MANDAT` → montant plus bas) regroupés en bloc → Orange (6×), AXA (5×), SFR (3×), Canal+, MMA désormais détectés avec le bon montant | Sur le PDF réel : 14 abonnements (avant : « 2026 » x110), sandbox `parser-releve-multiligne` 7/7, suite 661 PASS 0 FAIL, lint 0, build vert |
| 2026-07-14 | Le « + » abonnement complété (retour Juju) : ① `looksLikeStatement(text)` (pur) détecte un **relevé bancaire** → « Analyser le relevé → » (multi-détection `/analyse`) au lieu d'un abonnement unique ; ② facture unique → bouton **« ✉️ Résilier cet abonnement »** → `/resiliation?service=…` (résiliation en ligne si connue, sinon lettre + lien La Poste recommandé, déjà en place) | Sandbox 654 PASS 0 FAIL (`subscriptions-extract` +4 : relevé vs facture), lint 0, **build prod vert** |
| 2026-07-14 | Le « + » **écrit directement dans `subscriptions`** : `extractSubscriptionDraft(text)` (pur, `src/lib/subscriptions/extract.ts`) → brouillon nom/montant/fréquence, confirmé puis `createSubscription()` (store) → `POST /api/subscriptions` (socle, `source:'manual'`) / localStorage en invité ; UI `/ajouter` (formulaire éditable) | Sandbox 650 PASS 0 FAIL (`subscriptions-extract` 10/10), lint 0, **build prod vert** |
