...
export function ScreenLanding() {
  ...
  return (
    <div className="flex flex-col items-center text-center gap-0 animate-fade-up">
      {/* Badge */}
      <div className="inline-flex items-center gap-2 bg-sage/10 border border-sage/20 rounded-full px-4 py-2 mb-6 font-mono text-xs text-moss tracking-widest">
      ...
      {/* Headline */}
      <h1 className="font-serif text-[clamp(32px,7.5vw,68px)] leading-[1.06] tracking-[-0.03em] text-ink mb-5">
        Vous payez pour<br />
        des choses que vous<br />
        <em className="text-moss">n&apos;utilisez plus.</em>
      </h1>

      {/* Sub */}
      <p className="text-[clamp(15px,3.5vw,17px)] leading-[1.7] text-ink/70 font-light max-w-[420px] mb-2">
        Serein détecte vos abonnements oubliés et vos dépenses invisibles.
      </p>
      <p className="text-sm text-ink/50 italic mb-9">Et personne ne vous prévient.</p>
      ...
      <div className="flex flex-col items-center gap-2.5 w-full">
        ...
        <p className="font-mono text-[11px] text-ink/50 tracking-wider mt-1">
      ...
      <div className="flex gap-2.5 flex-wrap justify-center mt-8">
        {['Lecture seule', 'Données en France', '0 commission cachée'].map(t => (
          <span key={t} className="flex items-center gap-1.5 text-[11.5px] text-ink/50 bg-surface border border-ink/10 px-3.5 py-1.5 rounded-full">
