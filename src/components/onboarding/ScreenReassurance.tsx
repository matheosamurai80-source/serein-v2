...
export function ScreenReassurance() {
  ...
  return (
    <div className="flex flex-col items-center w-full animate-fade-up">
      ...
      <h2 className="font-serif text-[clamp(24px,5.5vw,44px)] tracking-[-0.025em] leading-[1.15] text-warm mb-6 text-center">
        Votre banque reste<br /><em className="text-sage-light">entièrement la vôtre.</em>
      </h2>

      {/* DSP2 card */}
      <div className="w-full bg-sage/7 border border-sage/16 rounded-2xl p-7 mb-5">
        <p className="font-serif text-lg tracking-[-0.01em] text-warm mb-4">
          Ce que la connexion DSP2 signifie concrètement
        </p>
        <div className="flex flex-col gap-3">
          {DSP2_ITEMS.map(item => (
            <div key={item.title} className="flex items-start gap-3 text-[13.5px] text-white/65 leading-[1.55]">
              <span className="w-[22px] h-[22px] rounded-full bg-sage/18 border border-sage/24 flex items-center justify-center text-[11px] text-sage font-bold flex-shrink-0 mt-0.5">✓</span>
              <span><strong className="text-warm">{item.title}</strong> — {item.desc}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5 w-full mb-5">
        {TRUST_TILES.map(tile => (
          <div key={tile.title} className="bg-white/3 border border-white/7 rounded-2xl p-4">
            <span className="text-lg mb-2 block">{tile.icon}</span>
            <p className="text-sm font-semibold text-warm mb-1">{tile.title}</p>
            <p className="text-xs text-white/38 leading-[1.55]">{tile.desc}</p>
          </div>
        ))}
      </div>

      <div className="w-full bg-white/3 border border-white/7 rounded-2xl p-4 mb-6 text-sm text-white/65 leading-[1.6] text-left">
        Vous préférez ne pas connecter votre banque ?{' '}
        <strong className="text-sage-light">Importez simplement un relevé PDF.</strong>{' '}
